<x-admin-layout>
    ...
    <div class="container" style="max-width: 900px; margin: 0 auto; padding: 20px;">
        <h1 style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin-bottom: 1.5rem;">Edit Purchase Order: {{ $po->no_po }}</h1>
        ...
        <div class="form-card">
            <form action="{{ route('admin.po.update', $po->id) }}" method="POST">
                ...
                <div style="margin-bottom: 1.5rem;">
                    <label for="no_po" class="form-label">Nomor Purchase Order (No. PO):</label>
                    <input type="text" 
                           name="no_po" 
                           id="no_po" 
                           value="{{ old('no_po', $po->no_po) }}"
                           class="form-input" 
                           style="{{ $errors->has('no_po') ? 'border-color: #dc2626;' : '' }}"
                           required 
                           autofocus>
                    @error('no_po')
                ...
                <div style="display: flex; align-items: center; justify-content: flex-end; border-top: 1px solid #e5e7eb; padding-top: 1.5rem; margin-top: 1.5rem;">
                    ...
                    <button type="submit" class="btn-submit">
                        <i class="fa-solid fa-save" style="margin-right: 0.5rem;"></i> Perbarui PO
                    </button>
                </div>
            </form>
        </div>
