<x-admin-layout>
    ...
    <div class="card">
        <h2 class="card-header">Edit Data User: {{ $kelolauser->name }}</h2>
        <form action="{{ route('admin.kelolauser.update', $kelolauser->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-grid">
                <div class="form-group">
                    <label for="name">Nama Lengkap</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $kelolauser->name) }}" required>
                    @error('name') <span class="error-text">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    <label for="username">Username</label>
                    <input type="text" name="username" id="username" value="{{ old('username', $kelolauser->username) }}" required>
                ...
                <div class="form-group col-span-2">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $kelolauser->email) }}">
                    @error('email') <span class="error-text">{{ $message }}</span> @enderror
                </div>
                <div class="form-group col-span-2">
                    <label for="role">Role</label>
                    <select name="role" id="role" required>
                        <option value="karyawan" {{ old('role', $kelolauser->role) == 'karyawan' ? 'selected' : '' }}>Karyawan</option>
                        <option value="team leader" {{ old('role', $kelolauser->role) == 'team leader' ? 'selected' : '' }}>Team Leader</option>
                    </select>
            ...
            <div class="form-footer">
                ...
                <button type="submit" class="btn btn-red">Update User</button>
            </div>
        </form>
    </div>
</x-admin-layout>